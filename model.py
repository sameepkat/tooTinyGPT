...
class GPT(nn.Module):
    def __init__(self, config: Config):
        ...
        self.position_embedding_table = nn.Embedding(
            self.block_size, embedding_dimension
        )
    ...
    def forward(
    ...
    ):  # x.shape = (B, T), y(B, T) = Correct next tokens
        """
        ...
        assert T <= self.block_size, (
            f"Cannot forward sequence length {T}, block_size is {self.block_size}"
        )
...
class Block(nn.Module):
    def __init__(self, config: Config):
        ...
        self.first_layernorm = nn.LayerNorm(
            config.n_embd, bias=False
        )  # normalize for attention
        self.attention = CausalSelfAttention(config)
        self.second_layernorm = nn.LayerNorm(
            config.n_embd, bias=False
        )  # attention block changes the representation, so
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Q = q.reshape(
            B, -1, n_head, head_size
        )  # Split d_model into n_head * head_size = C -> n_head * head_size
        K = k.reshape(B, -1, n_head, head_size)
        V = v.reshape(B, -1, n_head, head_size)

        Q = Q.permute(
            0, 2, 1, 3
        )  # bring n_head to front: (batch_size, n_head, seq_len , head_size)
        K = K.permute(
            0, 2, 1, 3
        )  # turns out permute is similar to transpose that allows multiple_dimensions like we are doing here
        V = V.permute(0, 2, 1, 3)

        K_T = K.permute(
            0, 1, 3, 2
        )  # from the dot attention formula: We need QK^T: Swapping last two dimensions
        ...
        attention = nn.functional.softmax(
            masked_scores, dim=-1
        )  # attention = (QK^T)/sqrt(d_k) * V # softmax over last dimension
        attention = self.attention_dropout(attention) @ V

        output = attention.permute(
            0, 2, 1, 3
        )  # transpose back to (batch_size, seq_len, n_head, head_size)
        reshaped_output = output.reshape(
            B, T, C
        )  # now i guess head should be (batch_size, seq_len, )
...
class FeedForward(nn.Module):
    def __init__(self, config: Config):
        ...
        hidden_size = (
            4 * config.n_embd
        )  # to allow larger hidden layer more capacity to transform each token vector

        self.ff_expand_layer = nn.Linear(
